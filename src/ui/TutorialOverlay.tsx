...
const STAGE_INFO: Partial<
  Record<AgeStage, { title: string; subtitle: string }>
> = {
  2: { title: "Stage 2", subtitle: "Thirst & new foods unlocked!" },
  3: { title: "Stage 3", subtitle: "Fireflies unlocked!" },
};
  ...
  2: [
    {
      callouts: [
        ...
        {
          target: "new-food",
          label: "New food feeds more, but slower recharge",
          below: true,
        },
      ],
      celebration: true,
  ...
  useEffect(() => {
    ...
    const targets = defs
      .flatMap(
        (c) =>
          Array.from(
            document.querySelectorAll(`[data-tutorial="${c.target}"]`),
          ) as HTMLElement[],
      )
      .filter(Boolean);
    targets.forEach((el) => {
  ...
  return (
    <>
      ...
      <div className={styles.dismissHint}>
        {isLastStep ? "tap to play" : "tap to continue"}
      </div>
    </>
  );
}
